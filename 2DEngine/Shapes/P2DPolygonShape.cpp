#include <2DEngine/Shapes/P2DPolygonShape.h>
#include <new>

inline P2DPolygonShape::P2DPolygonShape()
{
    m_type = Polygon;
...
void P2DPolygonShape::SetPoints(const P2DVec2* vertices, int32 count)
{
    ...
    int32 n = P2DMin(count, b2_maxPolygonVertices);

    // Merge points if they are too close
    P2DVec2 points[b2_maxPolygonVertices];
    int32 tempCount = 0;
    for (int32 i = 0; i < n; ++i)
    {
        ...
        {
            if (P2DDistanceSquared(v, points[j]) < 0.5f * b2_linearSlop)
            {
                unique = false;
                break;
            }
...
void P2DPolygonShape::SetARect(float32 hx, float32 hy)
{
	m_count = 4;
...
void P2DPolygonShape::SetARect(float32 hx, float32 hy, const P2DVec2& center, float32 angle)
{
	m_count = 4;
	m_vertices[0].Set(-hx, -hy);
	m_vertices[1].Set( hx, -hy);
...
int32 P2DPolygonShape::GetChildCount() const
{
    return 1;
}

bool P2DPolygonShape::TestPoint(const P2DTransform& transform, const P2DVec2& p) const
{
...
/*
bool P2DPolygonShape::RayCast(b2RayCastOutput* output, const b2RayCastInput& input,
...
/*
void P2DPolygonShape::ComputeAABB(b2AABB* aabb, const P2DTransform& transform, int32 childIndex) const
{
    NOT_USED(childIndex);
...
void P2DPolygonShape::ComputeMass(P2DMass* massData, float32 density) const
{
...
inline const P2DVec2& P2DPolygonShape::GetVertex(int32 index) const
{
    assert(0 <= index && index < m_count);
    return m_vertices[index];
}

bool P2DPolygonShape::ValidateConvexity() const
{
	for (int32 i = 0; i < m_count; ++i)
	{
		int32 i1 = i;
...
P2DVec2 P2DPolygonShape::GetCentroid(const P2DVec2* vs, int32 count)
{
    assert(count >= 3);
